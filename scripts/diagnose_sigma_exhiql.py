"""σ diagnostic for an EX-HIQL checkpoint. Mirror of diagnose_sigma.py
but imports EXCHIQLAgent instead of CHIQLAgent. Works with Phase-3a/3b
(indep trunks) and shared-trunk EX-HIQL checkpoints alike.

Adds one extra measurement over the original: σ-by-stochasticity-proxy.
We don't have the teleport zone mask directly, so we use a data-driven
proxy: for each (s,g) pair, compute σ and correlate with a per-state
"next-state entropy" proxy computed from the DATASET itself (how
variable is s' for (s, a) bins near this state). If σ concentrates
where dataset entropy is high, the signal is state-dependent in the
right way.

That's a heavier addition — for now we only add the proxy if
--stochasticity-proxy is set. Without it, this script behaves identically
to diagnose_sigma.py but loads an EX-HIQL checkpoint.
"""
import argparse
import json
import os
