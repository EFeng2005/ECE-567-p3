#!/usr/bin/env bash
# Status report for the 3-seed EX-HIQL (Phase 3 Design A) run.
set -u
BASE=/home/y_f/ece567/runs/ex_chiql_phase3/dummy/ex_chiql_phase3
...
# Check if all three seeds have a train row and eval row at step 1000000 + saved ckpt.
all_done=1
for s in 0 1 2; do
  d=$(ls -d "$BASE"/sd00${s}_* 2>/dev/null | head -1)
