"""Expectile-Heads C-HIQL with gradient clipping (EX-HIQL-clip).

`wide-tau-clip` branch variant: same 5 independent MLPs as indep-trunk
EX-HIQL (Phase-3a/3b), plus `optax.clip_by_global_norm(grad_clip)` in
the optimizer chain. Exposed as a new hyperparameter `grad_clip` (default
10.0). Tests whether grad clipping rescues the wide-τ runaway feedback
loop documented in PHASE3_DESIGN_A_REPORT.md §4.2-4.3.

Intended to be run in parallel with different `head_expectiles`:
  - (0.1, 0.3, 0.5, 0.7, 0.9) — wide, Phase-3a config
  - (0.4, 0.5, 0.6, 0.7, 0.8) — moderate
Everything else is identical to ex_chiql.py on the same branch.
...
class EXCHIQLAgent(flax.struct.PyTreeNode):
    ...
    def create(
        ...
        # Gradient clipping before Adam — the defining change of this variant.
        # Caps the per-step global gradient norm, bounding how far any single
        # head's parameters can move in a single step regardless of loss
        # magnitude. Intended to break the per-head runaway feedback loop that
        # drove Phase-3a's divergence at extreme τ.
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            # wide-tau-clip default: clip global grad norm at 10.0 before Adam.
            # See PHASE3_DESIGN_A_REPORT.md §7.2 for the rationale.
            grad_clip=10.0,
