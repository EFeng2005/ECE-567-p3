...
class GCValue(nn.Module):
    ...
    def setup(self):
        mlp_module = MLP
        if self.ensemble:
            mlp_module = ensemblize(mlp_module, self.num_ensemble)
    ...
    def __call__(self, observations, goals=None, actions=None, goal_encoded=False):
        ...
        Args:
            ...
            goal_encoded: Whether `goals` is already an encoded goal representation.
                When True, the gc_encoder skips its concat_encoder (same semantics as
                GCActor's goal_encoded flag). Default False preserves HIQL/GCIQL behavior.
        """
        if self.gc_encoder is not None:
            inputs = [self.gc_encoder(observations, goals, goal_encoded=goal_encoded)]
        else:
