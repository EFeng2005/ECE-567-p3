...
at the same fixed $(s, g)$ we visualise, collapses almost all 1000
sampled subgoal proposals onto the teleport-out tile.
Under EX-HIQL the high-level actor $\pi_\text{high}$ is trained
bit-identically to HIQL (Design A), so the raw sampled distribution
should remain pathological---the fix lives at the \emph{scoring} step,
not at the sampling step.
Diag~7 visualises this directly.

\paragraph{Setup.}
For the same $(s, g)$ pair used in Diag~3, we sample
$N = 1000$ subgoal representations from EX-HIQL Phase-3b's
$\pi_\text{high}(\cdot \mid s, g)$ and produce two density plots:
\begin{enumerate}
  \item \textbf{Panel A (raw):} the unmodified sample density, equivalent
        to Diag~3 but on the Phase-3b checkpoint.
  \item \textbf{Panel B (scored):} after scoring each candidate $z_k$ by
        $\bar V(s, z_k, g) - \beta_\text{pes}\,\sigma^\perp(s, z_k, g)$
        (the residual-$\sigma$ rule of Eq.~\eqref{eq:residual_sigma} with
        $\beta_\text{pes} = 0.5$) and retaining only the top-$K$
        ($K = 50$) candidates before the nearest-neighbour decode to XY.
\end{enumerate}
Diag~3's hypothesis was that the $\pi_\text{high}$ sampler
\emph{proposes} teleport-out-collapsed candidates; Diag~7's hypothesis is
that EX-HIQL's scoring step \emph{rejects} them.

\paragraph{Predicted result and paper update.}
We expect Panel A to reproduce Diag~3's teleport-out collapse
(confirming $\pi_\text{high}$ is unchanged by the scoring-rule
modification), and Panel B to redistribute the surviving candidates
toward the maze region the agent must actually traverse to reach $g$.
If observed, the contrast between Panel~A and Diag~3's
Figure~\ref{fig:diag3_density} would constitute a direct visual
counterpart to the disease-figure/cure-figure framing.
The experiment takes approximately two to three hours of scripting and
the residual decode; results will be added to this section in the
